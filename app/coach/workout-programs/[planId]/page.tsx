...
import { AddWorkoutDrawer } from "@/components/coach/workout-builder/CoachWorkoutDrawer";
...
function draftFromProgram(p: CoachWorkoutProgramRow): MetaDraft {
  return {
...
/** The coach program endpoints return `is_mine` and `can_edit` at the top
 *  level of the response — not on `program`. Merge them onto the program object
 *  so downstream code (readOnly checks, badges, etc.) sees them consistently. */
function mergeOwnership<T extends object>(
  program: T,
  res: { is_mine?: boolean; can_edit?: boolean },
): T & { is_mine?: boolean; can_edit?: boolean } {
  return {
    ...program,
    is_mine: res.is_mine,
...
export default function CoachWorkoutProgramDetailPage() {
  ...
  const [program, setProgram] = useState<CoachWorkoutProgramRow | null>(null);
  ...
  const isMine = program?.is_mine ?? false;
  const readOnly = !!program && (program.can_edit === false || !isMine);

  const dirty =
    !!program &&
    !!draft &&
    !readOnly &&
    !draftEquals(draft, draftFromProgram(program));
  ...
  const runEquipmentSync = useCallback(async () => {
    ...
    try {
      const sync = await CoachService.recomputeProgramEquipment(planId);
      if (planIdRef.current !== planId) return;
      if (sync.success && sync.program) {
        // Preserve ownership from the current program row — the recompute
        // endpoint doesn't necessarily re-emit is_mine / can_edit, and losing
        // them would flip the page into read-only mode.
        setProgram((prev) => ({
          ...sync.program,
          is_mine: sync.is_mine ?? prev?.is_mine,
          can_edit: sync.can_edit ?? prev?.can_edit,
        }));
      } else {
  ...
  const fetchWorkouts = useCallback(async () => {
    if (!planId) return;
    const wr = await CoachService.listWorkoutTemplates({
      plan_id: planId,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [pr, wr] = await Promise.all([
        CoachService.getWorkoutProgram(planId),
        CoachService.listWorkoutTemplates({
          plan_id: planId,
          include_equipment_adapted: false,
          page: 1,
          page_size: 10,
      ...
      if (pr.success && pr.program) {
        setProgram(mergeOwnership(pr.program, pr));
      } else {
        setError(pr.error || "Program not found");
      }
      setLoading(false);
      if (!cancelled && pr.success && pr.program && pr.is_mine) {
  ...
  const handleSave = async () => {
    if (!planId || !draft || !dirty || readOnly) return;
    if (!draft.plan_name.trim()) {
      alert("Name is required.");
      return;
    }
    setSaving(true);
    const res = await CoachService.updateWorkoutProgram(planId, buildPatch());
    setSaving(false);
    if (res.success && res.program) {
      setProgram((prev) => ({
        ...res.program,
        is_mine: res.is_mine ?? prev?.is_mine,
        can_edit: res.can_edit ?? prev?.can_edit,
      }));
    } else {
  ...
  const togglePublish = async () => {
    if (!planId || !program || readOnly) return;
    ...
    const patch = dirty
      ...
      : { status: nextStatus };
    const res = await CoachService.updateWorkoutProgram(planId, patch);
    setPublishing(false);
    if (res.success && res.program) {
      setProgram((prev) => ({
        ...res.program,
  ...
  const deleteProgram = async () => {
    ...
    if (
      ...
      return;
    const res = await CoachService.deleteWorkoutProgram(planId);
    if (res.success) router.push("/coach/workout-programs");
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      {!loading && program && draft && (
        <div className="space-y-8">
          {/* Read-only banner for programs the coach doesn't own */}
          {readOnly && (
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-800 mt-0.5">
                  It belongs to the shared library or another coach. You can
                  view its structure but not make changes.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            {/* Progress (only meaningful for owned programs) */}
            {!readOnly && (
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
            ...
            >
              {(program.equipment_required ?? []).length === 0 ? (
                ...
                (program.equipment_required ?? []).map((eq: string) => (
                  <li
                    key={eq}
        ...
        {slot.workout ? (
          <div className="flex flex-1 items-center justify-between gap-4 px-5 py-4">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <Link
                href={`/coach/workout-templates/${slot.workout.id}`}
