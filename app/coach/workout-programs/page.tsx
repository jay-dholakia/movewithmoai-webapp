...
import type { CoachEnrichedWorkoutProgramRow } from "@/lib/types/workout-builder";
import {
  ArrowRight,
  BookOpen,
  CalendarDays,
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        ...
        >
          <Shimmer className="h-4 flex-2 max-w-50" />
          <Shimmer className="h-4 flex-1 max-w-30" />
...
export default function CoachWorkoutProgramsPage() {
  const [programs, setPrograms] = useState<CoachEnrichedWorkoutProgramRow[]>(
    [],
  );
  ...
  const load = useCallback(async () => {
    ...
    if (res.success && Array.isArray(res.programs)) {
      setPrograms(res.programs as CoachEnrichedWorkoutProgramRow[]);
  ...
  const filterMoaiOptions = useMemo(() => {
    ...
    for (const p of programs) {
      for (const fm of p.focus_moais) m.set(fm.id, fm.name);
    }
    return [...m.entries()]
      .map(([id, name]) => ({ id, name }))
      .sort((a, b) =>
  ...
  const visiblePrograms = useMemo(() => {
    let out = sortedPrograms;
    if (ownership === "mine") out = out.filter((p) => p.is_mine);
    if (filterMoaiId)
      out = out.filter((p) =>
        p.focus_moais.some((fm) => fm.id === filterMoaiId),
      );
    return out;
  }, [sortedPrograms, filterMoaiId, ownership]);

  const mineCount = useMemo(
    () => programs.filter((p) => p.is_mine).length,
    [programs],
  );
      ...
      <header className="flex flex-col gap-6 border-b border-gray-200 pb-8 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl space-y-3">
          ...
          <p className="text-sm leading-relaxed text-gray-600">
            Programs you&apos;ve built plus everything in the shared library.
            You can create new programs or duplicate any existing one as a
            starting point for your own.
          </p>
        </div>
        <Link
          href="/coach/workout-programs/new"
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-gray-600">
              ...
              {mineCount > 0 && (
                <>
                  {" "}
                  · <span className="text-gray-500">{mineCount} yours</span>
                </>
              )}
            </p>
            <Link
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {visiblePrograms.map((p) => {
                      ...
                      const href = `/coach/workout-programs/${encodeURIComponent(p.plan_id)}`;
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {p.is_mine ? (
                                  ...
                                  <span
                                    ...
                                    title="Read-only. Duplicate to edit."
                                  >
                            ...
                            {p.focus_moais.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {p.focus_moais.map((fm) => (
                                  ...
                                  >
                                    <Target className="h-2.5 w-2.5 opacity-60" />
                                    {fm.name}
                                  </button>
                                ))}
                              </div>
                            ...
                            <div className="flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
                              ...
                              {!p.is_mine && (
                                <span
                                  ...
                                  title="Duplicate to your library from the program page"
                                >
      ...
      {!loading && !error && programs.length === 0 && (
        <div className="mt-12 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/60 px-6 py-14 text-center sm:px-12">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-gray-600">
            Create your first program or duplicate one from the shared library
            to start assigning training to your moais and members.
          </p>
          <Link
            href="/coach/workout-programs/new"
