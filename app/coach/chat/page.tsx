...
type ChatType = "client" | "moai" | "focus-moai";
...
function SignedMedia({
  ...
  useEffect(() => {
    ...
    // Storage path — get signed URL from moai-chat-media bucket
    supabase.storage
      .from("moai-chat-media")
  ...
  if (loading) {
    return (
      <div className="max-w-xs h-32 bg-gray-100 rounded-lg flex items-center justify-center">
  ...
  if (!url) {
    return (
      <div className="max-w-xs rounded-lg bg-gray-100 p-3 text-sm text-gray-500">
        Media unavailable
      </div>
    );
  }
  ...
  return <VideoPlayer src={url} messageId={messageId} />;
}
  ...
  useEffect(() => {
    ...
    const loadVideo = async () => {
      try {
        ...
        const firstBytesText = String.fromCharCode(
          ...uint8Array.slice(0, Math.min(50, uint8Array.length)),
        );
        const isMultipart =
          firstBytesText.includes("--") &&
          (firstBytesText.includes("WebKitFormBoundary") ||
            firstBytesText.includes("form-data"));

        if (isMultipart) {
          console.warn(
            "⚠️ Video file appears to be stored as multipart/form-data. This file may not play correctly. Re-upload the video to fix this.",
          );
          setError(
            "This video was stored incorrectly and cannot be played. Please re-upload the video.",
          );
          setLoading(false);
          return;
        }
        ...
        const firstBytes = String.fromCharCode(
          ...uint8Array.slice(0, Math.min(10, uint8Array.length)),
        );
        const isLikelyJSON =
        ...
        if (isLikelyJSON) {
          ...
          try {
            ...
            setError(
              `Server error: ${jsonData.message || jsonData.error || "Invalid response"}`,
            );
            setLoading(false);
            return;
          } catch {
        ...
        const first20BytesHex = Array.from(
          uint8Array.slice(0, Math.min(20, uint8Array.length)),
        )
        ...
        if (
          ...
          console.warn(
            "WebM file but signature check failed - using video/webm type anyway",
          );
          finalType = "video/webm";
        }
    ...
    <video
      ...
      onError={(e) => {
        ...
        if (err) {
          ...
          errorInfo.note =
            "Video error occurred but MediaError object is null/undefined";
        }
        ...
        if (err) {
          switch (err.code) {
            ...
            case MediaError.MEDIA_ERR_DECODE:
              errorMsg =
                "Video decode error - file may be corrupted or format not supported. The video file may be stored incorrectly (multipart/form-data instead of raw binary).";
              break;
          ...
          errorMsg =
            "Video failed to load (no error details available) - file may be corrupted or in wrong format";
        }

        setError(
          `Video error: ${errorMsg} ${err ? `(Code: ${err.code})` : ""}`,
        );
      }}
      onLoadStart={() => {
...
export default function ChatPage() {
  ...
  // Client chat state
  const [clientChatSessions, setClientChatSessions] = useState<
  ...
  const [moaiChatMessages, setMoaiChatMessages] = useState<
    Record<string, MoaiChatMessage[]>
  >({});
  const [moaiDetails, setMoaiDetails] = useState<Record<string, any>>({});
  const [focusMoaiChatIds, setFocusMoaiChatIds] = useState<
    Record<string, string>
  >({});
  const [focusMoaiMessages, setFocusMoaiMessages] = useState<
    Record<string, any[]>
  >({});
  ...
  const [recordedVideoPreview, setRecordedVideoPreview] = useState<
    string | null
  >(null);
  ...
  const loadData = async () => {
    try {
      ...
      const { data: focusMoaisData } = await supabase
        .from("focus_moais")
        .select(`id, name, workout_focus(name)`)
        .eq("coach_id", profile.id)
        .eq("status", "active");

      const focusMoaisWithMembers = await Promise.all(
        (focusMoaisData || []).map(async (fm: any) => {
      ...
      const focusMoaiChatItems: ChatListItem[] = focusMoaisWithMembers
        .filter((fm) => fm.member_count > 0)
        .map((fm) => ({
          id: fm.id,
          type: "focus-moai" as ChatType,
          name: fm.name,
          subtitle: `${fm.member_count} ${fm.member_count === 1 ? "member" : "members"} · Focus Moai`,
          unreadCount: 0,
          lastMessageAt: null,
          data: fm,
        }));
  ...
  const loadFocusMoaiChat = async (focusMoaiId: string) => {
    if (focusMoaiMessages[focusMoaiId]) return;
    ...
    if (!chatData) return;
    setFocusMoaiChatIds((prev) => ({ ...prev, [focusMoaiId]: chatData.id }));

    const { data: msgs } = await supabase
      .from("focus_moai_chat_messages")
      .select(
        `id, content, created_at, sender_id, is_coach, media_type, media_url, media_note, users(first_name, last_name, profile_picture_url)`,
      )
      .eq("chat_id", chatData.id)
      .order("created_at", { ascending: true })
      .limit(100);

    setFocusMoaiMessages((prev) => ({ ...prev, [focusMoaiId]: msgs || [] }));

    // Realtime
    supabase
      .channel(`focus_moai_coach_chat_${chatData.id}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
        ...
        async (payload: any) => {
          const { data } = await supabase
            ...
            .single();
          if (data) {
            setFocusMoaiMessages((prev) => {
              const existing = prev[focusMoaiId] || [];
              if (existing.some((m) => m.id === data.id)) return prev;
              return { ...prev, [focusMoaiId]: [...existing, data] };
            });
          }
        },
      )
  ...
  const handleSelectChat = (chat: ChatListItem) => {
    ...
    } else if (chat.type === "focus-moai") {
      loadFocusMoaiChat(chat.id);
    } else {
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        console.log("Recording stopped, creating blob from chunks:", {
          ...
          totalSize: recordingChunksRef.current.reduce(
  ...
  const attachRecordedVideo = () => {
    if (recordedVideo && recordedVideoPreview) {
      ...
      if (
        ...
        console.warn(
          "Recorded video blob had invalid type, forcing correct mime type:",
          {
  ...
  const recordAgain = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        if (isCancelingRef.current) {
          console.log(
            "Recording canceled (recordAgain), not processing chunks",
          );
          recordingChunksRef.current = [];
          return;
        }
        ...
        console.log(
          "Recording stopped (recordAgain), creating blob from chunks:",
          {
            chunkCount: recordingChunksRef.current.length,
            totalSize: recordingChunksRef.current.reduce(
              (sum, chunk) => sum + chunk.size,
  ...
  const cancelRecording = () => {
    ...
    if (mediaRecorderRef.current && isRecording) {
      try {
        ...
        console.log(
          "Error stopping recorder (expected if already stopped):",
          err,
        );
      }
    }
  ...
  const handleSendMessage = async () => {
    if (
      (!chatInputText.trim() && !selectedMedia) ||
      !selectedChat ||
      !coachProfile ||
      sendingMessage ||
      uploadingMedia
    ...
    try {
      ...
      if (selectedChat.type === "client") {
        ...
        const newMessage = await ChatService.sendMessage(
          ...
          coachProfile.id,
          chatInputText.trim() ||
            (mediaUrl ? (mediaType === "image" ? "📷 Image" : "🎥 Video") : ""),
          undefined,
          mediaUrl,
          mediaType,
        );
      ...
      } else if (selectedChat.type === "focus-moai") {
        const chatId = focusMoaiChatIds[selectedChat.id];
        if (!chatId) return;
  ...
  const currentMessages = selectedChat
    ? selectedChat.type === "client"
      ...
      : selectedChat.type === "focus-moai"
        ? focusMoaiMessages[selectedChat.id] || []
                  ...
                  <div className="flex items-start gap-3">
                    {chat.avatar ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                        {chat.type === "moai" || chat.type === "focus-moai" ? (
                          <Users
                            className={`h-6 w-6 ${chat.type === "focus-moai" ? "text-purple-500" : "text-gray-500"}`}
                          />
                        ) : (
              ...
              <div className="space-y-4">
                {currentMessages.length === 0 ? (
                  ...
                  currentMessages.map((message: any) => {
                    const isCoach =
                      message.is_coach || message.sender_type === "coach";
                    const senderName =
                      message.sender_name ||
                      (isCoach ? "You" : message.sender_username || "User");
                        ...
                        <div className={`max-w-md ${isCoach ? "order-2" : ""}`}>
                          ...
                          >
                            {(() => {
                              ...
                              const msgMediaType =
                                message.media_type ||
                                (message as any).media_type;

                              if (!msgMediaUrl || !msgMediaType) return null;
            ...
            <div className="bg-white border-t border-gray-200 p-4">
              ...
              {showRecordModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
                  <div
                    className="relative bg-black rounded-lg overflow-hidden shadow-2xl pointer-events-auto"
                    style={{
                      width: "640px",
                      maxWidth: "90vw",
                      aspectRatio: "16/9",
