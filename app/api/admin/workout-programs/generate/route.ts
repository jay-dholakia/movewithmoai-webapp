...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, difficulty_level, is_paid = false } = body;
    ...
    if (exError) {
      console.error("[generate] exercises fetch:", exError);
      return NextResponse.json(
    ...
    // Build compact catalog string to minimize tokens
    ...
    const systemPrompt = `You are a fitness programming expert. Create a 5-day workout program based on the user's request.
...
Return ONLY valid JSON. No markdown, no explanation.`;
    ...
    // Collect valid exercise IDs for validation
    ...
    if (pgError) {
      console.error("[generate] program insert:", pgError);
      return NextResponse.json(
    ...
    for (const w of generated.workouts) {
      ...
      if (wError) {
        console.error("[generate] workout insert:", wError);
        continue;
      }
      ...
      if (validExercises.length > 0) {
        ...
        if (exInsertError) {
          console.error("[generate] exercises insert:", exInsertError);
        }
      }
    }
    ...
    const { data: allExData } = await admin
      ...
      .in(
        "workout_template_id",
        (
          await admin.from("workoutss").select("id").eq("plan_id", planId)
        ).data?.map((w) => w.id) ?? [],
      );
    ...
    if (allExData) {
      ...
      for (const row of allExData) {
        const equip = (row as { exercise?: { equipment?: string[] } }).exercise
          ?.equipment;
