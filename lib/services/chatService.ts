...
export class ChatService {
  ...
  static async uploadChatMedia(
    ...
    chatType: "client" | "moai" | "focus-moai",
  ): Promise<string | null> {
    try {
      ...
      // Convert File to ArrayBuffer and upload directly
      // Supabase Storage requires binary data (ArrayBuffer) with explicit contentType
      // Uploading File or Blob objects can result in multipart/form-data wrapping
      const arrayBuffer = await file.arrayBuffer();

      console.log("Uploading as ArrayBuffer with content type:", {
        fileName: file.name,
        fileType: file.type,
      ...
      if (error) {
        ...
        if (error.message?.includes("Bucket not found")) {
          throw new Error(
            'Storage bucket "coach-chat-media" not found. Please create it in Supabase Storage settings.',
          );
        }
        return null;
      }
  ...
  static async sendMoaiChatMessage(
    ...
    if (mediaUrl) {
      if (mediaUrl.includes("/storage/v1/object/public/coach-chat-media/")) {
        storagePath = mediaUrl.split(
          "/storage/v1/object/public/coach-chat-media/",
        )[1];
    ...
    if (mediaUrl) {
      ...
      if (error) {
        ...
        console.log(
          "✅ Successfully called new RPC function with media, response:",
          data,
        );
      }
    }
    ...
    if (!data || error) {
      ...
      const result = await supabase.rpc("send_moai_chat_message_for_coach", {
        p_moai_chat_id: moaiChatId,
        p_coach_user_id: coachId,
        p_message: message,
        p_reply_parent_message_id: replyParentId || null,
    ...
    if (
      pathToUseForUpdate &&
      (!newMessage.media_storage_path || !newMessage.media_type)
    ) {
    ...
    console.log("Returning message with media:", {
      ...
      path_starts_with_client:
        newMessage.media_storage_path?.startsWith("client/"),
    });
          ...
          if (
            ...
            try {
              if (newMessage.is_coach) {
                ...
                if (coachError) {
                  console.error(
                    "Error fetching coach for message:",
                    coachError,
                  );
                }
                ...
                onNewMessage({
                  ...newMessage,
                  media_url: ChatService.getMediaUrl(
                    newMessage.media_storage_path,
                  ),
