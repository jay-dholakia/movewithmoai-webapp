...
export function FocusSelect({ value, onChange, supabase }: FocusSelectProps) {
  const [options, setOptions] = useState<WorkoutFocusOption[]>([]);
  ...
  const handleSelect = (opt: WorkoutFocusOption) => {
    onChange(opt.id);
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className={value ? "text-slate-800" : "text-slate-400"}>
          {value
            ? selectedName || "Selected focus"
            : "Select a workout focus..."}
        </span>
        <svg
