...
export function CoachAssignmentSection({
  ...
  const loadMoais = useCallback(async () => {
    ...
    try {
      const res = await fetchJSON(
        `/api/coach/focus-moais?page=${moaiPage}&page_size=${PAGE_SIZE}`,
      );
      if (res.success) {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchJSON(`/api/coach/paid-users?${q}`);
      if (res.success) {
        setUsers(res.users ?? []);
    ...
    <div className="rounded-lg border border-blue-200 bg-blue-50/40 p-4 space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          Pick a focus moai you coach, or an individual user in your circles.
        </p>
      </div>
      ...
      {assignType === "focus_moai" && (
        <div className="rounded-lg border border-gray-200 bg-white divide-y divide-gray-100 overflow-hidden">
          {moaiLoading ? (
            ...
            <EmptyRow text="No focus moais assigned to you." />
          ) : (
            moais.map((m) => (
              <PickerRow
                key={m.id}
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg border border-gray-200 bg-white divide-y divide-gray-100 overflow-hidden">
            {userLoading ? (
              ...
              <EmptyRow text="No users in your coaching scope." />
            ) : (
              users.map((u) => {
                const name =
                  u.display_name ||
